...
window.onload = function () {
	...
	CreateTable.prototype.init = function () {
		...
		var cells = '',
		    // Это строка, в которую будем вписывать ячейки
	...
	CreateTable.prototype.addFood = function () {
		...
		this.foodInterval = setInterval(function () {
			...
			if (cellClassList.length == 1) {
				// Смотрим на количество классов
	...
	CreateSnake.prototype.init = function () {
		...
		for (var i = 0; i < this.body.length; i++) {
			...
			this.cell[(this.body[i].x - 1) * this.table.col + this.body[i].y - 1].classList.add(this.color);
		}
	...
	CreateSnake.prototype.createScoreBox = function () {
		...
		this.count = scoreList.querySelectorAll('.js-scoreCount__item')[document.body.querySelectorAll('.js-scoreCount__item').length - 1].querySelector('.js-scoreCount__count');
	...
	CreateSnake.prototype.setNextCell = function () {
		...
		if (this.newHeadClasslist.contains(this.table.foodImage)) {
			// Съедаем фрукт
			...
			document.querySelector('.js-scoreCount__count[data-id=' + this.color + ']').innerHTML = this.food;
	...
	CreateSnake.prototype.die = function () {
		...
		for (var i = 0; i < this.body.length; i++) {
			...
			this.cell[(this.body[i].x - 1) * this.table.col + this.body[i].y - 1].classList.remove(this.color, 'head');
		}
	...
	document.querySelector('.js-init').addEventListener('click', function () {
		...
		var input = document.querySelector('.js-level:checked'),
		    speed = input.value * 1,
		    // Скорость движения всех змеек на данной карте
	...
	document.querySelector('.js-toggleSound').addEventListener('click', function () {
	...
	document.querySelector('.js-lostBox__send').addEventListener('click', function () {
